...
object Gen {
  ...
  object Domain {
    ...
    def map2[A, B, C](d1: Domain[A], d2: Domain[B])(f: (A, B) => C): Domain[C] = {
      val f1: (Option[A], Option[B]) => Option[C] = (oa, ob) =>
        (oa, ob) match {
    ...
    def flatMap[A, B](d: Domain[A])(f: A => Domain[B]): Domain[B] = {
      val f1: Option[A] => Stream[Option[B]] = x =>
        x match {
  ...
  case class Gen[+A](sample: State[RNG, A], exhaustive: Domain[A]) { self =>
    def map[B](f: A => B): Gen[B] =
      self.copy(sample = sample.map(f), exhaustive = Domain.map(exhaustive)(f))
  ...
  def listOfN[A](n: Int, g: Gen[A]): Gen[List[A]] = {
    def go(n: Int, gs: Gen[List[A]]): Gen[List[A]] =
      if (n == 0) gs
      else {
        val ngs = for {
          a <- g
